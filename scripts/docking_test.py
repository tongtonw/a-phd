...
#
# Xtest = input_d
# err_m = error_gen(Xtest, 'error_m_docking0')

err_m = pd.read_csv('../error_m_docking0.csv')
...
plt.figure()
plt.plot(Dz.iloc[:-1, [3]].values, err_m['error_u'], '.')
...
plt.figure()
plt.subplot(3, 1, 1)
plt.plot(err_m.iloc[:, 4], err_m.iloc[:, 7], '.')
