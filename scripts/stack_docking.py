...
n_path = 2
data_m = pd.read_csv('../error_m_docking{}.csv'.format(n_path)).values
data_gp = pd.read_csv('../err_gp_xy_test_docks{}.csv'.format(n_path)).values

X_1 = data_m[:, 13].reshape(-1, 1)
...
# testY_pre = model.predict(X_test)
# plt.figure()
# plt.plot(testY_pre)
#
...
# testwd = 1 - testwp
#
# weightYtest = testwp * X_test[:,0] + testwd * X_test[:,1]
#
# plt.figure()
# plt.plot(X_test[:,0], label='model')
# plt.plot(X_test[:,1], label='gp')
# plt.plot(weightYtest, label='weighted')
# plt.plot(y_test, label='true')
# plt.legend()
#
# testm_e = data_m[:, 16].reshape(-1, 1)
# testd_e = data_gp[:, 7].reshape(-1, 1)
# testw_e = weightYtest - y_test
#
# plt.figure()
